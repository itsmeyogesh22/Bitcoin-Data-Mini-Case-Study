...
SELECT 
	market_date,
	CEILING(MAX(volume)) AS max_volume_observed, 
	close_price
FROM daily_btc
...
SELECT 
	...
	CONCAT(
			(ROUND((COUNT(*)/(SELECT COUNT(DISTINCT market_date) FROM daily_btc)::NUMERIC)*100)
		), 
...
SELECT
high_days,
ROUND(100 * high_days / total_days::NUMERIC) AS proportion_of_total_days
FROM cte;
...
FROM daily_btc
WHERE 
	market_date = '2021-01-01'::DATE
)
SELECT 
	ROUND((T2.ending_price * T1.btc_volume), 2) AS final_returns,
	ROUND(((T2.ending_price - T1.starting_price)/T1.starting_price)*100, 2) AS rate_of_return
FROM 
	start_dt AS T1,
	end_dt AS T2;